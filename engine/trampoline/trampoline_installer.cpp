...
void* TrampolineInstaller::CreateNativeBridgeTrampoline(void* callback, void* backup) {
    void* mem = Memory::AllocUnprotected(kNativeBridgeTrampolineSize);
    if (UNLIKELY(!mem)) {
        LOGE("Failed to allocate native bridge trampoline!");
        return nullptr;
    }
    memcpy(mem, kNativeBridgeTrampoline, kNativeBridgeTrampolineSize);
    uintptr_t addr = reinterpret_cast<uintptr_t>(mem);

    auto origin_entry_out = reinterpret_cast<void**>(addr + kNativeBridgeTrampolineOriginEntryOffset);
    *origin_entry_out = backup;

    auto callback_out = reinterpret_cast<void**>(addr + kNativeBridgeTrampolineCallbackOffset);
    *callback_out = callback;

    Memory::FlushCache(mem, kNativeBridgeTrampolineSize);
    return mem;
}

void* TrampolineInstaller::InstallNativeInlineTrampoline(art::ArtMethod* target, void* callback, bool skip_first_few_bytes) {
    void* target_code_addr = target->GetCompiledCodeAddr();
    bool target_code_writable = Memory::Unprotect(target_code_addr);
    if (UNLIKELY(!target_code_writable)) {
        LOGE("Failed to make target code writable!");
    ...
    void* native_bridge = CreateNativeBridgeTrampoline(callback, backup);
    if (UNLIKELY(!native_bridge)) return nullptr;

    {
        ScopedMemoryAccessProtection protection(target_code_addr, kDirectJumpTrampolineSize);
        if (skip_first_few_bytes) {
            FillWithNopImpl(target_code_addr, kSkipBytes);
            WriteDirectJumpTrampolineTo(AS_VOID_PTR(AS_PTR_NUM(target_code_addr) + kSkipBytes), native_bridge);
        } else {
            WriteDirectJumpTrampolineTo(target_code_addr, native_bridge);
        }
    }

    target->AfterHook(true, target->IsNative());
    std::atomic_thread_fence(std::memory_order_seq_cst);
    Memory::FlushCache(target, sizeof(art::ArtMethod));
    return backup;
}

void* TrampolineInstaller::InstallNativeReplacementTrampoline(art::ArtMethod* target, void* callback) {
    void* origin_code_entry = target->GetEntryPointFromCompiledCode();
    void* native_bridge = CreateNativeBridgeTrampoline(callback, origin_code_entry);
    if (UNLIKELY(!native_bridge)) return nullptr;

    target->SetEntryPointFromCompiledCode(native_bridge);
    target->AfterHook(false, target->IsNative());

    // Ensure memory visibility and flush cache across CPU cores
    std::atomic_thread_fence(std::memory_order_seq_cst);
    Memory::FlushCache(target, sizeof(art::ArtMethod));
...
void* TrampolineInstaller::InstallNativeDirectReplacement(art::ArtMethod* target, void* replacement_func) {
    void* target_code_addr = target->GetCompiledCodeAddr();
    void* backup = nullptr;
    if (target_code_addr && !CannotSafeInlineHook(target)) {
        Memory::Unprotect(target_code_addr);
        backup = Backup(target, kDirectJumpTrampolineSize);
        {
            ScopedMemoryAccessProtection protection(target_code_addr, kDirectJumpTrampolineSize);
            WriteDirectJumpTrampolineTo(target_code_addr, replacement_func);
        }
        target->AfterHook(true, target->IsNative());
    } else {
        backup = target->GetEntryPointFromCompiledCode();
        void* direct_jump = CreateDirectJumpTrampoline(replacement_func);
        target->SetEntryPointFromCompiledCode(direct_jump);
        target->AfterHook(false, target->IsNative());
    }
    std::atomic_thread_fence(std::memory_order_seq_cst);
    Memory::FlushCache(target, sizeof(art::ArtMethod));
