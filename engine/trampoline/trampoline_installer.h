...
namespace pine {
    ...
    public:
        ...
        TrampolineInstaller(size_t skip_bytes) : kReplacementModeOnly(false), kSkipBytes(skip_bytes) {
        };

        TrampolineInstaller(size_t skip_bytes, bool replacement_only) : kReplacementModeOnly(replacement_only), kSkipBytes(skip_bytes) {
        }
        ...
        void Init() {
            ...
            kBackupTrampolineSize = SubAsSize(kNativeBridgeTrampoline, kBackupTrampoline);
            kNativeBridgeTrampolineSize = SubAsSize(kTrampolinesEnd, kNativeBridgeTrampoline);
        }
        ...
        size_t GetDirectJumpTrampolineSize() const {
            return kDirectJumpTrampolineSize;
        }
        ...
        bool CannotSafeInlineHook(art::ArtMethod* target) {
            // Inline hooking patches the method's compiled code in place. When the
            // method has no dedicated compiled code (never JIT/AOT compiled), its
            // entry_point_from_compiled_code_ points at a *shared* runtime bridge
            // (art_quick_to_interpreter_bridge / ExecuteNterpImpl). Patching that
            // shared bridge is unsafe (the size/instruction probes below then read
            // unrelated bytes and may conclude "safe") and ineffective (interpreted
            // callers may not route through the exact patched address), which shows
            // up as "hooked successfully but never intercepted". Force the
            // replacement trampoline, which rewrites the ArtMethod entry point.
            if (UNLIKELY(!target->IsCompiled())) {
                LOGW("Cannot safe inline hook method: target is not compiled yet "
                     "(entry is a shared bridge), using replacement trampoline");
                return true;
            }
        ...
        void* CreateNativeBridgeTrampoline(void* callback, void* backup);

        void* InstallNativeInlineTrampoline(art::ArtMethod* target, void* callback, bool skip_first_few_bytes);

        void* InstallNativeReplacementTrampoline(art::ArtMethod* target, void* callback);

        void* InstallNativeDirectReplacement(art::ArtMethod* target, void* replacement_func);
    ...
    protected:
        ...
        inline size_t NativeBridgeTrampolineOffset(void* ptr) {
            return SubAsSize(ptr, kNativeBridgeTrampoline);
        }
        ...
        void const* kNativeBridgeTrampoline;
        size_t kNativeBridgeTrampolineOriginEntryOffset;
        size_t kNativeBridgeTrampolineCallbackOffset;
        size_t kNativeBridgeTrampolineSize;
