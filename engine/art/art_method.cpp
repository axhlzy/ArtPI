...
void ArtMethod::InitMembersNative(JNIEnv* env) {
    if (Android::version >= Android::kN) {
        kAccCompileDontBother = (Android::version >= Android::kOMr1)
                                ? AccessFlags::kCompileDontBother_O_MR1
                                : AccessFlags::kCompileDontBother_N;
    ...
    access_flags_.SetOffset(GetDefaultAccessFlagsOffset());
    entry_point_from_jni_.SetOffset(GetDefaultEntryPointFromJniOffset());
    entry_point_from_compiled_code_.SetOffset(GetDefaultEntryPointFromQuickCompiledCodeOffset());

    if (android_version >= Android::kQ) {
        size = Android::Is64Bit() ? 32 : 24;
    } else if (android_version >= Android::kO) {
        size = Android::Is64Bit() ? 48 : 32;
    } else if (android_version >= Android::kM) {
        size = Android::Is64Bit() ? 48 : 36;
    } else {
        size = Android::Is64Bit() ? 56 : 40;
    }

    if (env) {
        jclass objClass = env->FindClass("java/lang/Object");
        if (objClass) {
            jmethodID m1 = env->GetMethodID(objClass, "wait", "()V");
            jmethodID m2 = env->GetMethodID(objClass, "wait", "(J)V");
            if (m1 && m2) {
                size_t diff = Difference(reinterpret_cast<intptr_t>(m1), reinterpret_cast<intptr_t>(m2));
                if (diff >= 16 && diff <= 128) {
                    size = diff;
                }
            }
    ...
    if (Android::version < Android::kN) {
        uint32_t entry_point_member_size = Android::version == Android::kL ? sizeof(uint64_t) : sizeof(void*);
        entry_point_from_interpreter_ = new Member<ArtMethod, void*>(
                entry_point_from_jni_.GetOffset() - entry_point_member_size);
    }
}
