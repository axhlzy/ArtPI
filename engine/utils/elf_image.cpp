...
void* ElfImage::GetSymbolAddress(const char* name, bool warn_if_missing) const {
    ...
    if (UNLIKELY(!offset)) {
        // Fallback to xDL lookup in loaded ELF (searches both .dynsym and .symtab with gnu_debugdata)
        if (elf_name_) {
            void* xdl_handle = xdl_open(elf_name_, XDL_DEFAULT);
            if (xdl_handle) {
                void* sym = xdl_sym(xdl_handle, name, nullptr);
                if (!sym) {
                    sym = xdl_dsym(xdl_handle, name, nullptr);
                }
...
void* ElfImage::GetModuleBase(const char* name) {
    // 1. Try xDL first (fast, reliable, does not depend on /proc/self/maps permissions)
    void* xdl_handle = xdl_open(name, XDL_DEFAULT);
    if (xdl_handle) {
        xdl_info_t info;
        if (xdl_info(xdl_handle, XDL_DI_DLINFO, &info) == 0 && info.dli_fbase) {
            xdl_close(xdl_handle);
    ...
    // 2. Fallback to /proc/self/maps
    FILE* maps = WrappedFOpen("/proc/self/maps", "re");
    if (!maps) {
        LOGE("Failed to open /proc/self/maps for %s", name);
        return nullptr;
    }
    char buff[256];
    off_t load_addr;
