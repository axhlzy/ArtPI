...
/*
 * Per-dispatch tracer hook: fires on EVERY instruction (before execution),
 * whether reached by linear flow, branch, or exception handler.
 * tctx == NULL keeps the interpreter overhead to two pointer checks.
 */
# define FINISH(_offset) {                                                  \
        ADJUST_PC(_offset);                                                 \
        inst = FETCH(0);                                                    \
        if (UNLIKELY(tctx != NULL)) {                                       \
            tctx->pc = (u4)(pc - code->insns);                              \
            tctx->inst = inst;                                              \
            tctx->regs = fp;                                                \
            tctx->regFlags = fp_flags;                                      \
            tctx->insns_executed++;                                         \
            if (tctx->tracer != NULL && tctx->tracer->onInsn != NULL &&     \
                tctx->tracer->onInsn(tctx) != 0) {                          \
                retval.j = 0;                                               \
                GOTO_bail();                                                \
            }                                                               \
        }                                                                   \
        if (UNLIKELY(g_vmTraceHook != NULL)) {                              \
            g_vmTraceHook(env, code, (u4)(pc - code->insns), inst,          \
                          fp, fp_flags);                                    \
        }                                                                   \
...
/*
 * Wrap a method invocation with the tracer's invoke policy:
 *   VM_INVOKE_JNI     -> execute `_x` normally
 *   VM_INVOKE_MOCK    -> retval = mock value, `_x` skipped
 *   VM_INVOKE_STEP_IN -> `_tr->stepIn` executes the callee, `_x` skipped
 *                        when it reports the call was handled
 */
# define TRACED_INVOKE(_receiver, _args, _argc, _x)                         \
    {                                                                       \
        bool _skipCall = false;                                             \
        const VmTracer *_tr = (tctx != NULL) ? tctx->tracer : NULL;         \
        if (_tr != NULL) { memset(&retval, 0, sizeof(retval)); }            \
        if (_tr != NULL && (_tr->onInvokePre != NULL ||                     \
                            _tr->stepIn != NULL)) {                         \
            jvalue _mockRet;                                                \
            memset(&_mockRet, 0, sizeof(_mockRet));                         \
            int _policy = VM_INVOKE_JNI;                                    \
            if (_tr->onInvokePre != NULL) {                                 \
                _policy = _tr->onInvokePre(tctx, methodToCall,              \
                        (int) INST_INST(inst), (jobject)(_receiver),        \
                        (int)(_argc), (const jvalue*)(_args), &_mockRet);   \
            } else if (_tr->stepIn != NULL) {                               \
                _policy = VM_INVOKE_STEP_IN;                                \
            }                                                               \
            if (_policy == VM_INVOKE_MOCK) {                                \
                retval = _mockRet;                                          \
                _skipCall = true;                                           \
            } else if (_policy == VM_INVOKE_STEP_IN &&                      \
                       _tr->stepIn != NULL) {                               \
                int _sr = _tr->stepIn(tctx, methodToCall,                   \
                                (int) INST_INST(inst), (jobject)(_receiver),\
                                (int)(_argc), (const jvalue*)(_args),       \
                                &retval);                                   \
                if (_sr == 0) {                                             \
                    _skipCall = true;                                       \
                } else if (_sr == 2) {                                      \
                    retval.j = 0;                                           \
                    GOTO_bail();     /* hand-off: abort whole run */        \
                }                                                           \
            }                                                               \
        }                                                                   \
        if (!_skipCall) { _x; }                                             \
        if (_tr != NULL && _tr->onInvokePost != NULL) {                     \
            _tr->onInvokePost(tctx, methodToCall,                           \
                              (int) INST_INST(inst), &retval,               \
                              wrapper->ExceptionCheck(env));                \
        }                                                                   \
    }
...
{                                                                              \
    ...
    TRACED_INVOKE(NULL, args,                                                  \
                  (int) strlen(paramTypes), _x);                               \
    /*                                                                         \
     * 如果申请的内存则需要释放                                                   \
     */                                                                        \
    if (args != args_tmp) {                                                    \
...
vmTraceHookFn g_vmTraceHook = NULL;
...
jvalue vmInterpret(
    ...
    return vmInterpret2(env, code, dvmResolver, NULL);
}
                    ...
                    switch (returnCh) {
                        ...
                        case 'L':
                            retval.l = wrapper->CallObjectMethodA(env,
                                                                  ...
                                                                  methodToCall->methodId,
                                                                  args + 1);
                            /* retval.l kept: consumed by move-result, or freed by the sandbox's PushLocalFrame (onInvokePost still needs it) */
                            break;
                        case 'V':
 ...
 */
    GOTO_TARGET(exceptionThrown)
    {
        ...
        if (exception.get() == NULL) {
            // diagnostics: no pending exception when we got here
            ALOGE("exceptionThrown with NULL exception at pc=0x%x inst=0x%04x",
                  (int)(pc - code->insns), INST_INST(FETCH(0)));
            GOTO_bail();
        }
 ...
 */
        ...
        if (tctx != NULL && tctx->tracer != NULL &&
            tctx->tracer->onException != NULL) {
            tctx->tracer->onException(tctx, exception.get(),
                                      (u4) (pc - code->insns), catchRelPc);
        }
